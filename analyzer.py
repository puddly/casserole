...
with serial.Serial(sys.argv[1], baudrate=115200) as ser:
    while True:
        try:
            header = ser.read_until(b'\xE1\xE2\xE3\xE2\xE1')

            if len(header) > 5:
                print(f'Wasted {len(header) - 5} bytes resyncing...')

            timestamp = int.from_bytes(ser.read(4), 'little')
            ...
            if crc16(inner_packet[:-4]) != int.from_bytes(checksum, 'big'):
                raise BadPacketError(f'Checksum is invalid: {crc16(inner_packet[:-4])} != {int.from_bytes(checksum, "big")}')

            if len(command_end) != 2:
                raise BadPacketError(f'Command end is too short {pretty_bytes(command_end)}, {len(command_end)} != 2')
            ...
            if 3 + len(payload + checksum + command_end) != total_packet_length:
                raise BadPacketError(f'Invalid packet length {3 + len(payload + checksum + command_end)} != {total_packet_length}')
